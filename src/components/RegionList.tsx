...
export function RegionList({ stores, favorites, onToggleFavorite, onSelect }: Props) {
  const pageSize = 5;
  ...
  const provinces = useMemo<ProvinceGroup[]>(() => {
    ...
    Object.values(group).forEach((p) => {
      const cityMap: Record<string, { city: string; dji: number; insta: number; stores: Store[] }> = {};
      stores
        .filter((s) => (s.province || '未标注') === p.province)
        .forEach((s) => {
          const city = s.city || '未标注';
          if (!cityMap[city]) cityMap[city] = { city, dji: 0, insta: 0, stores: [] };
          if (s.brand === 'DJI') cityMap[city].dji += 1;
          else cityMap[city].insta += 1;
          cityMap[city].stores.push(s);
        });
                        ...
                        <div key={c.city} className="rounded-2xl border border-slate-100 bg-slate-50/50 overflow-hidden">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setStorePage((prev) => ({ ...prev, [cityKey]: 0 }));
                            }}
                          >
                            <div className="w-5 text-[11px] text-slate-400 text-center">{(cityPage[p.province] ?? 0) * 5 + idx + 1}</div>
                          ...
                          {cityOpen && (
                            <div className="bg-gray-50 px-3 pb-3 space-y-2">
                              {c.stores
                                .slice((storePage[cityKey] ?? 0) * 8, (storePage[cityKey] ?? 0) * 8 + 8)
                                .map((s) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <button
                                        className={`p-2 rounded-full border ${isFav ? 'border-amber-300 text-amber-500 bg-amber-50' : 'border-slate-200 text-slate-300'}`}
                                        onClick={(e) => {
                                          e.stopPropagation();
                              ...
                              {Math.ceil(c.stores.length / 8) > 1 && (
                                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
                                  <button
                                    className="px-2 py-1 rounded-full border border-slate-200 disabled:opacity-40"
                                    disabled={(storePage[cityKey] ?? 0) === 0}
                                    onClick={() => setStorePage((prev) => ({ ...prev, [cityKey]: Math.max(0, (storePage[cityKey] ?? 0) - 1) }))}
                                  >
                                    上一页
                                  </button>
                                  <span>第 {(storePage[cityKey] ?? 0) + 1} / {Math.ceil(c.stores.length / 8)} 页</span>
                                  <button
                                    className="px-2 py-1 rounded-full border border-slate-200 disabled:opacity-40"
                                    disabled={(storePage[cityKey] ?? 0) >= Math.ceil(c.stores.length / 8) - 1}
                                    onClick={() =>
                                      setStorePage((prev) => ({
                                        ...prev,
                                        [cityKey]: Math.min(Math.ceil(c.stores.length / 8) - 1, (storePage[cityKey] ?? 0) + 1),
                                      }))
                                    }
                                  >
                                    下一页

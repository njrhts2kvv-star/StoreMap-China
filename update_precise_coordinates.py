...
def update_csv_coordinates(csv_path: Path, dry_run: bool = False):
    """
    更新CSV文件中的经纬度坐标
    ...
    if "name" not in df.columns or "city" not in df.columns or "brand" not in df.columns:
        print("[错误] CSV文件缺少 name、city 或 brand 列")
        return
    ...
    for idx, row in df.iterrows():
        ...
        print(f"\n[{idx + 1}/{total}] {brand} - {store_name} ({city})")
...
if __name__ == "__main__":
    ...
    try:
        update_csv_coordinates(CSV_FILE, dry_run=dry_run)

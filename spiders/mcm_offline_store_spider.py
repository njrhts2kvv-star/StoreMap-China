...
class MCMOfflineStoreSpider(BaseStoreSpider):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="MCM 门店爬虫（高德关键词）")
    parser.add_argument(
        "--output",
        "-o",
        default="各品牌爬虫数据/MCM_offline_stores.csv",
        help="输出文件路径",
    )
    args = parser.parse_args()

    spider = MCMOfflineStoreSpider()
    items = spider.fetch_items()
    spider.save_to_csv(items, args.output, validate_province=False)
    print(f"MCM 导出 {len(items)} 条门店")

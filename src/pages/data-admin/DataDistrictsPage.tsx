...
export default function DataDistrictsPage() {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader title="数据管理 · 商圈" description="商圈维度基础信息管理占位（API 只读）" />
      <TableCard title="商圈表" description="后续接入创建/编辑/删除接口">
        <table className="min-w-full text-sm">
          <thead className="text-left text-neutral-6">
            <tr>
              <th className="py-2">名称</th>
              <th className="py-2">城市码</th>
          ...
          <tbody>
            {districts.map((d) => (
              <tr key={d.id} className="border-t border-neutral-2">
                ...
                <td className="py-2">{d.cityCode || '-'}</td>
